...
import { checkExistingBeneficiary, createBeneficiaryDocument } from '../utils/firebase';
...
const BeneficiaryNew = () => {
    ...
    const handleSubmit = async (event) => {
        ...
        const existing = await checkExistingBeneficiary(birthDate, lastName, firstName);
        if (existing) {
            alert("error: benefeciary exists");
            return;
        }

        const response = await createBeneficiaryDocument(formFields);
        if (!response.includes("error")) {
            navigate("/beneficiaries/" + response);
        } else {
            alert(response);
        }
    }
