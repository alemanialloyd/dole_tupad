...
const ProjectBeneficiaryItem = ({ index, project, beneficiary, onAddHandler, additionalClasses, value, onValueChange }) => {
    ...
    return (
        <tr onClick={onAdd} className={additionalClasses ? additionalClasses : ""}>
            ...
            <td>{status === "pending" ? <input readOnly type="checkbox" checked={additionalClasses}/> : status === "ongoing" ? <FormInput id={index} onChange={(e) => {onValueChange(e)}} min="0" value={value} max={parseInt(days)} required style={{width: 100 + "px"}} type="number"/> : value}</td>
        </tr>
    );
}
